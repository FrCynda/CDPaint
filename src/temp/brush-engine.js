    /**
 * BRUSH ENGINE
 * Extracted from paintUlti_opt7.html
 *
 * This module implements PaintApp.brush — the full advanced brush pipeline.
 * All parameters are wired to the brush sidebar UI controls.
 *
 * Dependencies:
 *   - A global `PaintApp` object must exist before this script runs.
 *   - PaintApp.config.c1 / c1 / c2 — active foreground/background colours.
 *   - PaintApp.getActiveCanvas() — returns the drawing HTMLCanvasElement.
 *   - PaintApp.setActiveTab(tabName) — switches the ribbon tab.
 *   - PaintApp.pushUndo() — pushes current canvas state onto undo stack.
 *   - The HTML structure produced by brush_engine.html must be in the DOM.
 *
 * Usage:
 *   PaintApp.brush.activate()        // enable / disable the engine
 *   PaintApp.brush.isActive()        // true when engine is on
 *   PaintApp.brush.setParam(k, v)    // set any DEFAULTS key
 *   PaintApp.brush.loadPreset(name)  // 'round' | 'calligraphy' | ...
 *   PaintApp.brush.randomize()       // randomise all params
 *   PaintApp.brush.saveCurrentBrush()// open "Save brush" modal
 */
